...
## SFT 前后对比：Base Model vs SFT Step 100

SFT 在 5,119 条 MATH 蒸馏数据上训练 100 步后，在完整验证集上的表现：

### MATH（5,000 条验证集）

| 指标 | Base Model | SFT Step 100 | 变化 |
|------|-----------|-------------|------|
| **准确率** | 22.80% (1140/5000) | **54.50%** (2725/5000) | **+31.70pp** |
| **格式合规率** | 65.78% | **93.74%** | **+27.96pp** |

按学科分桶（Base → SFT → 提升）：

| 学科 | Base | SFT | 提升 |
|------|------|-----|------|
| Prealgebra | 34.9% | 68.0% | **+33.1pp** |
| Algebra | 28.6% | 73.8% | **+45.2pp** |
| Counting & Probability | 23.4% | 50.4% | +27.0pp |
| Geometry | 23.8% | 41.5% | +17.7pp |
| Number Theory | 19.8% | 49.4% | +29.6pp |
| Precalculus | 13.9% | 40.7% | +26.8pp |
| Intermediate Algebra | **9.7%** | 36.5% | **+26.8pp** |

按难度分桶（Base → SFT → 提升）：

| 难度 | Base | SFT | 变化 |
|------|------|-----|------|
| Level 1 | 40.3% | **83.8%** | **+43.5pp** |
| Level 2 | 32.3% | 72.5% | +40.2pp |
| Level 3 | 24.8% | 63.0% | +38.2pp |
| Level 4 | 19.7% | 50.7% | +31.0pp |
| Level 5 | **11.7%** | 28.9% | **+17.2pp** |

关键发现：
- **Algebra 提升最大**（+45pp），但 Prealgebra 基础最好（34.9%→68.0%）
- **Intermediate Algebra 最难**（Base 仅 9.7%，SFT 后 36.5%），几何也是弱项
- **Level 5 难题提升最小**（+17pp），说明蒸馏数据中难题通过率低，样本不够
- 各难度层级 SFT 均有 17-44pp 的提升，无负收益科目

### GSM8K（1,319 条测试集）

| 指标 | Base Model | SFT Step 100 | 变化 |
|------|-----------|-------------|------|
| **准确率** | 70.74% | **57.47%** | **-13.27pp** |

GSM8K 下降了 13pp——这是**灾难性遗忘（Catastrophic Forgetting）** 的典型表现。SFT 只在 MATH 数据上训练（r1_zero 格式），模型过度适应了 `<think> → <answer>` 的推理格式，忘记了 GSM8K 的 `####` 格式回答方式。这说明：

1. **SFT 数据多样性很重要**——只训一个 domain 会丢掉其他 domain 的能力
2. **GRPO 需要做 multi-task**——不能只优化 MATH，要同时保留 GSM8K
3. **后续可以混合 GSM8K SFT 数据**——7,473 条 GSM8K 的 CoT 数据可以补回来

### 结论

```
MATH 基线 22.5% → SFT 54.5%（+32pp，有效）
GSM8K 基线 70.7% → SFT 57.5%（-13pp，灾难性遗忘）

GRPO 的目标：
  MATH:  54.5% → 80%+
  GSM8K: 57.5% → 恢复 70%+（通过 multi-task 或 replay）
```
